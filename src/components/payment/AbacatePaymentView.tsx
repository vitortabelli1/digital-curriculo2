...
// Este componente gerencia o pagamento via Abacate Pay para qualquer plano
// (Básico ou Premium), reutilizando a mesma lógica: cria um checkout com
// completionUrl, redireciona a PRÓPRIA aba e, após o pagamento, o Abacate
// retorna para a URL configurada (?basic-paid=1 / ?premium-paid=1).
export function AbacatePaymentView({
  planId,
  returnParam,
  onBack,
}: {
  planId: PlanId;
  returnParam: "basic-paid" | "premium-paid";
  onBack: () => void;
}) {
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/abacate/checkout?plan=${encodeURIComponent(planId)}`,
          { method: "POST" }
        );
  ...
  // Após o pagamento, o Abacate retorna para a tela correspondente por meio do
  // completionUrl, sem depender de botão manual.
  const openPayment = () => {
    if (!payUrl) return;
        ...
        <div className="flex w-full flex-col items-center gap-6 rounded-2xl border border-zinc-200 bg-white p-8 text-center shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div>
            <h2 className="text-lg font-bold text-zinc-800 dark:text-zinc-100">
              Plano {plan.name} — R$ {plan.amount.toFixed(2).replace(".", ",")}
            </h2>
